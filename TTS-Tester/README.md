...
message, get an LLM reply (Google Gemini), hear it spoken back in a chosen
emotion. Built to test the engine against realistic, varied conversational
text instead of hand-picked one-off lines — pick `auto` and the LLM chooses
the emotional tone per reply itself, or pin a specific one from the dropdown.
